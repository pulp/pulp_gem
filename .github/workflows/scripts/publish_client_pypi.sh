...
export VERSION=$(ls dist | sed -rn 's/pulp_gem-client-(.*)\.tar.gz/\1/p')
...
twine upload dist/pulp_gem_client-$VERSION-py3-none-any.whl -u pulp -p $PYPI_PASSWORD
twine upload dist/pulp_gem-client-$VERSION.tar.gz -u pulp -p $PYPI_PASSWORD
