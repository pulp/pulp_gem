...
if [[ "$TEST" = "docs" ]]; then
  ...
  cmd_prefix bash -c "cd pulp_gem/docs; make diagrams html; tar -cvf docs.tar ./_build"
  docker cp pulp:/pulp_gem//docs/docs.tar docs.tar
...
if [[ "${RELEASE_WORKFLOW:-false}" == "true" ]]; then
  ...
  REPORTED_VERSION=$(http $STATUS_ENDPOINT | jq --arg plugin gem --arg legacy_plugin pulp_gem -r '.versions[] | select(.component == $plugin or .component == $legacy_plugin) | .version')
  response=$(curl --write-out %{http_code} --silent --output /dev/null https://pypi.org/project/pulp-gem/$REPORTED_VERSION/)
  if [ "$response" == "200" ];
  then
    echo "pulp_gem $REPORTED_VERSION has already been released. Skipping running tests."
    exit
  fi
fi
...
if [[ "$TEST" == "plugin-from-pypi" ]]; then
  ...
  git checkout ${COMPONENT_VERSION} -- pulp_gem/tests/
fi
...
cmd_prefix pip3 install /root/pulp-openapi-generator/pulp_gem-client
sudo rm -rf ./pulp_gem-client
...
# Run unit tests.
cmd_prefix bash -c "PULP_DATABASES__default__USER=postgres pytest -v -r sx --color=yes -p no:pulpcore --pyargs pulp_gem.tests.unit"

# Run functional tests
if [[ "$TEST" == "performance" ]]; then
  if [[ -z ${PERFORMANCE_TEST+x} ]]; then
    cmd_prefix bash -c "pytest -vv -r sx --color=yes --pyargs --capture=no --durations=0 pulp_gem.tests.performance"
  else
    cmd_prefix bash -c "pytest -vv -r sx --color=yes --pyargs --capture=no --durations=0 pulp_gem.tests.performance.test_$PERFORMANCE_TEST"
  fi
  exit
fi
...
if [ -f $FUNC_TEST_SCRIPT ]; then
  source $FUNC_TEST_SCRIPT
    ...
    if [[ "$GITHUB_WORKFLOW" == "Gem Nightly CI/CD" ]] || [[ "${RELEASE_WORKFLOW:-false}" == "true" ]]; then
        cmd_prefix bash -c "pytest -v -r sx --color=yes --suppress-no-test-exit-code --pyargs pulp_gem.tests.functional -m parallel -n 8 --nightly"
        cmd_prefix bash -c "pytest -v -r sx --color=yes --pyargs pulp_gem.tests.functional -m 'not parallel' --nightly"
        ...
        cmd_prefix bash -c "pytest -v -r sx --color=yes --suppress-no-test-exit-code --pyargs pulp_gem.tests.functional -m parallel -n 8"
        cmd_prefix bash -c "pytest -v -r sx --color=yes --pyargs pulp_gem.tests.functional -m 'not parallel'"
