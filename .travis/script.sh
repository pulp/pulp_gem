...
# './plugin-template --travis pulp_gem' to update this file.
#
...
if [ "$TEST" = 'bindings' ]; then
  ...
  ./generate.sh pulp_gem ruby 0

  cd pulp_gem-client
  gem build pulp_gem_client
  gem install --both ./pulp_gem_client-0.gem
  cd ..
...
if [[ "$TEST" == "performance" ]]; then
  ...
  if [[ -z ${PERFORMANCE_TEST+x} ]]; then
    travis-wait-enhanced --interval=1m --timeout=40m -- pytest -vv -r sx --color=yes --pyargs --capture=no --durations=0 pulp_gem.tests.performance
  else
    travis-wait-enhanced --interval=1m --timeout=40m -- pytest -vv -r sx --color=yes --pyargs --capture=no --durations=0 pulp_gem.tests.performance.test_$PERFORMANCE_TEST
  fi
  exit
fi
...
if [ -f $FUNC_TEST_SCRIPT ]; then
  source $FUNC_TEST_SCRIPT
    ...
    pytest -v -r sx --color=yes --pyargs pulp_gem.tests.functional
fi
