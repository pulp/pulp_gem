...
cd $TRAVIS_BUILD_DIR
export REPORTED_VERSION=$(http :24817/pulp/api/v3/status/ | jq --arg plugin pulp_gem -r '.versions[] | select(.component == $plugin) | .version')
...
GEM_FILE="$(ls | grep pulp_gem_client-)"
gem push ${GEM_FILE}
